...
func (md *StatsMiddleware) GenerateStats() Data {
	...
	totalRequestByStatus := make(map[string]int, len(md.totalRequestByStatus))
	for key, value := range md.totalRequestByStatus {
		totalRequestByStatus[key] = value
	}
	totalRequestByPath := make(map[string]int, len(md.totalRequestByPath))
	for key, value := range md.totalRequestByPath {
		totalRequestByPath[key] = value
	}
	...
	r := Data{
		...
		TotalRequestByStatus:   totalRequestByStatus,
		TotalRequestByPath:     totalRequestByPath,
	}
	return r
}
func (md *StatsMiddleware) GenerateStatsAndReset() Data {
	r := md.GenerateStats()
	md.Lock()
	defer md.Unlock()
	md.totalRequests = 0
